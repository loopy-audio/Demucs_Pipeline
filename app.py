...
def health_check():
    ...
    return jsonify({
        "status": "healthy",
        "message": "Stem separation API is running"
    })
...
def separate_audio():
    ...
    Returns: ZIP file containing 4 separated stems (bass.mp3, drums.mp3, vocals.mp3, other.mp3)
    """
    try:
        ...
        if len(stem_files) != 4:
            ...
            return jsonify({
                ...
                "message": f"Expected 4 stems, found {len(stem_files)}",
                "found_files": [f.name for f in stem_files]
            }), 500
        ...
        print(f"Separation complete: {filename} -> {len(stem_files)} stems ({zip_size:.1f} MB)")
